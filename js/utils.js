/* global NexT, CONFIG */
HTMLElement.prototype.wrap=function(e){this.parentNode.insertBefore(e,this),this.parentNode.removeChild(this),e.appendChild(this)},function(){const e=()=>document.dispatchEvent(new Event("page:loaded",{bubbles:!0}));document.addEventListener("DOMContentLoaded",e),document.addEventListener("pjax:success",e)}(),NexT.utils={scrollTo(e,t){window.CSS?.supports("scroll-behavior","smooth")?e.scrollTo({top:t,behavior:"smooth"}):e.scrollTo(0,t)},registerExtURL(){document.querySelectorAll("span.exturl").forEach(e=>{const t=document.createElement("a");
...
window.addEventListener("scroll",()=>{if(e||t){const n=document.body.scrollHeight-window.innerHeight,o=n>0?Math.min(100*window.scrollY/n,100):0;e&&(e.classList.toggle("back-to-top-on",Math.round(o)>=5),e.querySelector("span").innerText=Math.round(o)+"%"),t&&t.style.setProperty("--progress",o.toFixed(2)+"%")}this.updateActiveNav()},{passive:!0}),e?.addEventListener("click",()=>{NexT.utils.scrollTo(window,0)})},
/**
...
setTimeout(()=>{o.style.transition="",o.style.height=""},250)},1e3),!CONFIG.stickytabs)return;const u=n.parentNode.getBoundingClientRect().top+window.scrollY+10;NexT.utils.scrollTo(window,u)})}),window.dispatchEvent(new Event("tabs:register"))},registerCanIUseTag(){
...
return e.addEventListener("click",n=>{n.preventDefault();const o=t.getBoundingClientRect().top+window.scrollY;NexT.utils.scrollTo(window,o),history.pushState(null,document.title,e.href)}),t}),this.updateActiveNav()},registerPostReward(){const e=document.querySelector(".reward-container button");e&&e.addEventListener("click",()=>{document.querySelector(".post-reward").classList.toggle("active")})},activateNavByIndex(e){const t=document.querySelector(".post-toc:not(.placeholder-toc) .nav");if(!t)return;const n=t.querySelectorAll(".nav-item")[e];if(!n||n.classList.contains("active-current"))return;t.querySelectorAll(".active").forEach(e=>{e.classList.remove("active","active-current")}),n.classList.add("active","active-current");let o=n.querySelector(".nav-child")||n.parentElement,i=0;for(;t.contains(o);){if(o.classList.contains("nav-item"))o.classList.add("active");else{
// The last nav-item in a list has a margin-bottom of 5px.
i+=[...o.children].reduce((e,t)=>{const n=[...t.children].find(e=>e.classList.contains("nav-child"));return e+t.getBoundingClientRect().height-(n?.getBoundingClientRect().height||0)},0)+5,o.style.setProperty("--height",`${i}px`)}o=o.parentElement}
// Scrolling to center active TOC element if TOC content is taller then viewport.
const r=document.querySelector("Pisces"===CONFIG.scheme||"Gemini"===CONFIG.scheme?".sidebar-panel-container":".sidebar");document.querySelector(".sidebar-toc-active")&&NexT.utils.scrollTo(r,r.scrollTop-r.offsetHeight/2+n.getBoundingClientRect().top-r.getBoundingClientRect().top)},updateSidebarPosition(){if(window.innerWidth<1200||"Pisces"===CONFIG.scheme||"Gemini"===CONFIG.scheme)return;
